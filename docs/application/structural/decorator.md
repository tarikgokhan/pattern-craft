...
| Pattern | Decorator |
| Dosya Yolu | `docs/application/structural/decorator.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | Infrastructure, Application veya API sınırı |

## 1. Kısa Tanım

Decorator, mevcut nesneye dinamik olarak ek sorumluluklar ekler.
...
Talep servisinin etrafına validation, logging, cache veya retry gibi ek davranışlar eklenir.
...
DI registration sırasında aynı interface için zincirlenen decorator sınıflarıyla uygulanabilir.
...
```text
Client -> Logging Decorator -> Validation Decorator -> Core Service
```

## 6. Örnek Kod / Taslak

```csharp
public sealed class LoggingRequestServiceDecorator : IRequestService
{
    private readonly IRequestService _inner;
    private readonly ILogger<LoggingRequestServiceDecorator> _logger;

    public LoggingRequestServiceDecorator(
        IRequestService inner,
        ILogger<LoggingRequestServiceDecorator> logger)
    {
        _inner = inner;
        _logger = logger;
    }

    public async Task<Result> SubmitAsync(Guid requestId, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Submitting request {RequestId}", requestId);
        return await _inner.SubmitAsync(requestId, cancellationToken);
    }
}
```
