...
| Dosya Yolu | `docs/application/dotnet-application/result-pattern.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Result Pattern, başarı/hata çıktısını exception dışı standart bir modelle temsil eder.
...
Beklenen iş kuralı hataları exception fırlatmadan standart başarı/hata modeliyle döndürülür.
...
`Result`, `Result<T>`, `Error` gibi modeller application ve domain sınırında kullanılabilir.
...
```text
Operation -> Result Success / Failure -> API Mapping
```
