...
| Dosya Yolu | `docs/application/dotnet-application/unit-of-work.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Unit of Work, birden fazla veri değişikliğini tek transaction kapsamında yönetir.
...
Talep güncelleme, audit log ve outbox kaydı aynı local transaction içinde commit edilir.
...
EF Core `DbContext` çoğu senaryoda Unit of Work görevi görür.
...
```text
Changes -> Unit of Work -> Commit / Rollback
```
