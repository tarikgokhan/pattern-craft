...
| Dosya Yolu | `docs/application/dotnet-application/options-pattern.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Options Pattern, konfigürasyonları strongly typed sınıflarla yönetir.
...
Bildirim, raporlama veya dış sistem bağlantı ayarları strongly typed options sınıflarına bağlanır.
...
`IOptions<T>`, `IOptionsSnapshot<T>` ve `IOptionsMonitor<T>` ile uygulanır.
...
```text
Configuration -> Options Class -> Service
```
