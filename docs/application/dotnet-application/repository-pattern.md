...
| Dosya Yolu | `docs/application/dotnet-application/repository-pattern.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Repository Pattern, veri erişim detaylarını application/domain katmanından soyutlar.
...
Talep aggregate’ını kalıcılık detaylarından izole ederek application/domain katmanını EF Core’dan bağımsız tutar.
...
`IRequestRepository` domain/application boundary’de, `RequestRepository` persistence katmanında yer alır.
...
```text
Application Handler -> Repository Interface -> Repository Implementation -> DbContext
```
