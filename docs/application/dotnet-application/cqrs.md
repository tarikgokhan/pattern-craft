...
Talep oluşturma/güncelleme işlemleri ile listeleme/raporlama sorguları ayrı modellerle yönetilir.
...
Command handler ve query handler ayrımıyla uygulanır; gerekirse MediatR kullanılabilir.
...
```text
Command Side -> Domain Write Model | Query Side -> Read Model
```

## 6. Örnek Kod / Taslak

```text
CreateRequestCommand  -> CreateRequestCommandHandler
SearchRequestsQuery   -> SearchRequestsQueryHandler
```
