...
| Dosya Yolu | `docs/application/dotnet-application/application-service.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Application Service, use-case seviyesindeki uygulama akışlarını yönetir.
...
Use-case orchestration; repository çağrısı, domain davranışı, transaction ve event hazırlığı application service tarafından koordine edilir.
...
`SubmitRequestUseCase`, `ApproveRequestApplicationService` gibi isimlerle uygulanabilir.
...
```text
API -> Application Service -> Domain + Repository + Events
```
...
{
    public Task<Result> ExecuteAsync(Guid requestId, CancellationToken cancellationToken)
    {
        // Orchestrate use-case steps.
