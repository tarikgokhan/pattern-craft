...
| Dosya Yolu | `docs/application/dotnet-application/validation-pipeline.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Validation Pipeline, komut ve istekleri merkezi doğrulama zincirinden geçirir.
...
Command handler çalışmadan önce request model ve command validasyonları merkezi olarak uygulanır.
...
FluentValidation + MediatR pipeline behavior ile yaygın uygulanır.
...
```text
Command -> Validation Behavior -> Handler
```
