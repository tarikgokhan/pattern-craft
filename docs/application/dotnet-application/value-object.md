...
| Dosya Yolu | `docs/application/dotnet-application/value-object.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Value Object, kimliği olmayan, değeriyle anlam kazanan domain nesnelerini modeller.
...
Talep önceliği, açıklama, başlık veya süre gibi kimliksiz ama kurallı değerler value object olarak modellenir.
...
Immutable record/class, equality by value ve validation içeren küçük domain nesneleriyle uygulanır.
...
```text
Primitive Value -> Value Object -> Encapsulated Rule
```

## 6. Örnek Kod / Taslak

```csharp
public sealed record RequestTitle
{
    public RequestTitle(string value)
    {
