...
| Dosya Yolu | `docs/application/dotnet-application/domain-service.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Domain Service, tek bir entity içine koyulamayan domain davranışlarını taşır.
...
Tek bir entity içine doğal olarak yerleşmeyen domain davranışları domain service ile modellenir.
...
`RequestAssignmentDomainService` veya `ApprovalPolicyDomainService` gibi sınıflar domain katmanında kalır.
...
```text
Multiple Domain Objects -> Domain Service -> Domain Decision
```

## 6. Örnek Kod / Taslak

```csharp
public sealed class RequestAssignmentDomainService
{
    public AssignedTeam DetermineTeam(Request request, IReadOnlyCollection<TeamCapacity> capacities)
    {
        return capacities.OrderByDescending(x => x.AvailableSlots).First().Team;
    }
}
```
