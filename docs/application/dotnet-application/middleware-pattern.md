...
| Dosya Yolu | `docs/application/dotnet-application/middleware-pattern.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Middleware Pattern, hTTP request pipeline üzerinde ortak davranışları yönetir.
...
Exception handling, correlation id, logging ve authentication gibi ortak HTTP davranışları pipeline’a eklenir.
...
ASP.NET Core middleware zinciri bu desenin doğrudan karşılığıdır.
...
```text
HTTP Request -> Middleware A -> Middleware B -> Endpoint -> Response
```
