...
| Dosya Yolu | `docs/application/dotnet-application/domain-event.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | ASP.NET Core ve Clean Architecture katmanları |

## 1. Kısa Tanım

Domain Event, domain içinde oluşan önemli olayları event olarak modeller.
...
Talep onaylandığında domain içinde oluşan önemli olay `RequestApprovedDomainEvent` olarak modellenir.
...
Aggregate içinde event toplanır, Unit of Work sonrası handler’lar çalıştırılır.
...
```text
Aggregate Behavior -> Domain Event -> Event Handler
```
