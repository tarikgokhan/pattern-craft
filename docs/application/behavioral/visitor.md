...
| Pattern | Visitor |
| Dosya Yolu | `docs/application/behavioral/visitor.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | Application ve Domain davranışları |

## 1. Kısa Tanım

Visitor, nesne yapısını değiştirmeden yeni operasyonlar eklemeyi sağlar.
...
Farklı talep, onay ve audit nesneleri üzerinde raporlama veya export operasyonları eklenir.
...
Nesne ağacı sabit, operasyonlar sık değişiyorsa visitor kullanılabilir.
...
```text
Object Structure -> Visitor -> New Operation
```

## 6. Örnek Kod / Taslak

```csharp
public interface IRequestElement
{
...
public interface IRequestVisitor
{
    ...
    void Visit(Approval approval);
    void Visit(AuditEntry auditEntry);
}
```
