...
| Pattern | Interpreter |
| Dosya Yolu | `docs/application/behavioral/interpreter.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | Application ve Domain davranışları |

## 1. Kısa Tanım

Interpreter, basit dil, ifade veya kural yapısını yorumlar.
...
Dinamik rapor filtreleri veya basit kural ifadeleri yorumlanarak sorgu kriterlerine çevrilir.
...
`RequestFilterExpressionParser` veya `RuleExpressionInterpreter` gibi sınıflarla uygulanabilir.
...
```text
Expression Text -> Parser -> Expression Object -> Evaluation
```

## 6. Örnek Kod / Taslak

```text
priority == 'High' AND status == 'WaitingForApproval'
        ↓
