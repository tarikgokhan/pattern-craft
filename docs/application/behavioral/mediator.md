...
| Pattern | Mediator |
| Dosya Yolu | `docs/application/behavioral/mediator.md` |
| Odak | Tek uygulama / tek mikroservis içi kod mimarisi |
| Önerilen Katman | Application ve Domain davranışları |

## 1. Kısa Tanım

Mediator, nesneler arası doğrudan bağımlılığı merkezi aracıyla azaltır.
...
Controller doğrudan handler veya servisleri bilmeden command/query mesajını mediator’a gönderir.
...
MediatR veya benzeri bir mediator ile command/query handler ayrımı kurulur.
...
{
    var result = await _mediator.Send(new SubmitRequestCommand(id, User.Identity!.Name!), cancellationToken);
