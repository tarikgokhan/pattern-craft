...
| Pattern | Retry |
| Dosya Yolu | `docs/microservice/resilience/retry.md` |
...
Retry, geçici hatalarda işlemi kontrollü şekilde tekrar dener.
...
Geçici network veya servis yoğunluğu hatalarında istek kontrollü şekilde tekrar denenir.
...
Exponential backoff ve jitter ile uygulanmalıdır.
...
```text
Call -> Temporary Failure -> Wait -> Retry
```
