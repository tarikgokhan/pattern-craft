...
| Pattern | Fallback |
| Dosya Yolu | `docs/microservice/resilience/fallback.md` |
...
Fallback, ana akış çalışmadığında alternatif cevap veya davranış sağlar.
...
Ana kaynak çalışmazsa cache, varsayılan cevap veya kısıtlı mod devreye girer.
...
Kullanıcı deneyimini kontrollü şekilde sürdürür.
...
```text
Primary Call Failed -> Fallback Response
```
