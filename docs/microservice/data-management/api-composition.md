...
| Dosya Yolu | `docs/microservice/data-management/api-composition.md` |
...
API Composition, dağıtık veriyi birden fazla servisten toplayarak cevap oluşturur.
...
Dashboard ekranı için talep özeti, onay bekleyenler ve son audit kayıtları farklı servislerden toplanabilir.
...
BFF veya composition service seviyesinde uygulanır.
...
```text
Dashboard API -> RequestService + ApprovalService + AuditService
```
