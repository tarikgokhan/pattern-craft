...
| Pattern | CQRS |
| Dosya Yolu | `docs/microservice/data-management/cqrs.md` |
...
CQRS, mikroservislerde okuma ve yazma modellerini ayırır.
...
Yazma tarafı Request aggregate ile çalışırken, okuma tarafı raporlama için optimize edilmiş read model kullanır.
...
Command ve query veri modelleri farklı ihtiyaçlara göre ayrılır.
...
```text
Write Model -> Domain Aggregate | Read Model -> RequestSummaryView
```
