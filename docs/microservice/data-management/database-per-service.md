...
| Alt Kategori | Data Management Patterns |
| Pattern | Database per Service |
| Dosya Yolu | `docs/microservice/data-management/database-per-service.md` |
...
Database per Service, her servisin kendi veritabanına sahip olmasını sağlar.
...
RequestService kendi veritabanına, ReportingService kendi okuma deposuna, NotificationService kendi mesaj kayıtlarına sahip olur.
...
Servislerin veri sahipliğini netleştirir; cross-service join yapılmaz.
...
```text
RequestService -> RequestDb | ReportingService -> ReportingDb
```
