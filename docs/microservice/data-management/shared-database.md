...
| Dosya Yolu | `docs/microservice/data-management/shared-database.md` |
...
Shared Database, geçiş dönemlerinde birden fazla servisin aynı veritabanını kullanmasıdır.
...
Legacy geçişte bazı servisler geçici olarak aynı veritabanını okuyabilir; hedef mimari değildir.
...
Geçiş taktiği olarak kısa süreli ve kontrollü kullanılmalıdır.
...
```text
LegacyDb -> RequestService + ReportingService (temporary)
```
