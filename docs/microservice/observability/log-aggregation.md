...
| Dosya Yolu | `docs/microservice/observability/log-aggregation.md` |
...
Log Aggregation, log’ları merkezi bir yerde toplar.
...
Tüm servislerin structured log kayıtları merkezi platformda toplanır.
...
Dağıtık sistemde hata analizi ve operasyonel takip için temel gereksinimdir.
...
```text
Services -> Log Collector -> Central Log Store
```
