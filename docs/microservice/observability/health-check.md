...
| Dosya Yolu | `docs/microservice/observability/health-check.md` |
...
Health Check, servisin çalışabilirlik ve hazırlık durumunu bildirir.
...
Servisin ayakta olup olmadığı ve bağımlılıklarının hazır olup olmadığı bildirilir.
...
Kubernetes readiness/liveness veya monitoring sistemleri tarafından kullanılır.
...
```text
Probe -> Health Endpoint -> Healthy/Unhealthy
```
