...
| Alt Kategori | Observability Patterns |
| Pattern | Distributed Tracing |
| Dosya Yolu | `docs/microservice/observability/distributed-tracing.md` |
...
Distributed Tracing, bir isteğin servisler arasındaki yolculuğunu izler.
...
Bir isteğin gateway’den başlayıp servisler arasında nasıl ilerlediği trace-id ile izlenir.
...
Dağıtık çağrı zincirlerinde gecikme ve hata noktası bulunur.
...
```text
Trace Context -> Service A -> Service B -> Service C
```
