...
| Alt Kategori | Distributed Transaction Patterns |
| Pattern | Compensating Transaction |
| Dosya Yolu | `docs/microservice/distributed-transaction/compensating-transaction.md` |
...
Compensating Transaction, hata durumunda önceki başarılı adımları telafi eder.
...
Bir akışın sonraki adımı başarısız olursa önceki başarılı adım ters işlemle telafi edilir.
...
Tam geri alma mümkün değilse iş seviyesinde dengeleme aksiyonu tasarlanır.
...
```text
Step A Success -> Step B Failed -> Compensate Step A
```
