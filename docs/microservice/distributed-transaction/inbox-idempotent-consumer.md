...
| Dosya Yolu | `docs/microservice/distributed-transaction/inbox-idempotent-consumer.md` |
...
Inbox / Idempotent Consumer, aynı mesajın tekrar işlenmesini engeller.
...
Aynı mesaj tekrar geldiğinde consumer ikinci kez işlem üretmez.
...
Message broker tekrar teslimat davranışları için zorunlu güvenlik ağıdır.
...
```text
Incoming Message -> Inbox Check -> Process Once
```
