...
| Pattern | Saga |
| Dosya Yolu | `docs/microservice/distributed-transaction/saga.md` |
...
Saga, dağıtık işlemleri yerel transaction ve telafi adımlarıyla yönetir.
...
Talep onay süreci birden fazla servis adımından oluşuyorsa her adım local transaction olarak yönetilir.
...
Başarısız adımlar telafi işlemleriyle dengelenir.
...
```text
Submit -> Reserve Workflow Slot -> Notify -> Update Report
```
