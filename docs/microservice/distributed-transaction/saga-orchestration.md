...
| Dosya Yolu | `docs/microservice/distributed-transaction/saga-orchestration.md` |
...
Saga Orchestration, merkezi orchestrator ile dağıtık akışı yönetir.
...
Merkezi orchestrator talep onay sürecindeki adımları sırasıyla yönetir.
...
Akış karmaşık ve izlenebilirlik önemliyse tercih edilir.
...
```text
RequestSagaOrchestrator -> Step A -> Step B -> Step C
```
