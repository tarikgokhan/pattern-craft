...
| Dosya Yolu | `docs/microservice/distributed-transaction/transactional-outbox.md` |
...
Transactional Outbox, dB kaydı ile event yayınlamayı güvenli hale getirir.
...
Talep DB’ye kaydedilirken publish edilecek event aynı transaction içinde outbox tablosuna yazılır.
...
DB commit başarılı ama event publish başarısız riskini azaltır.
...
```text
Local Transaction -> Domain Change + Outbox Message -> Publisher
```
