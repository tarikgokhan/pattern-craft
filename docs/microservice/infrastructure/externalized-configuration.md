...
| Dosya Yolu | `docs/microservice/infrastructure/externalized-configuration.md` |
...
Externalized Configuration, konfigürasyonun uygulama kodundan ayrılmasıdır.
...
Connection string, endpoint, timeout ve feature flag gibi ayarlar kod dışına alınır.
...
Aynı build farklı ortamlarda farklı config ile çalışabilir.
...
```text
Code + External Config -> Runtime Settings
```
