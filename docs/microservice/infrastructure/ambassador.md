...
| Pattern | Ambassador |
| Dosya Yolu | `docs/microservice/infrastructure/ambassador.md` |
...
Ambassador, dış servis iletişimini temsilci proxy üzerinden yönetir.
...
Servisin dış çağrıları bir outbound proxy üzerinden yönetilir.
...
Retry, timeout, mTLS veya policy yönetimi kod dışına taşınabilir.
...
```text
Service -> Ambassador Proxy -> External Endpoint
```
