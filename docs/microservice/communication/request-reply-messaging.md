...
| Pattern | Request / Reply Messaging |
| Dosya Yolu | `docs/microservice/communication/request-reply-messaging.md` |
...
Request / Reply Messaging, mesajlaşma üzerinden istek-cevap modeli kurar.
...
Uzun süren entegrasyon simülasyonları için mesajla istek gönderilip sonuç event’i beklenebilir.
...
HTTP bağımlılığını azaltır ancak correlation yönetimi gerekir.
...
```text
RequestCommandMessage -> Worker -> RequestResultMessage
```
