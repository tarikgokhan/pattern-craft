...
| Dosya Yolu | `docs/microservice/communication/synchronous-communication.md` |
...
Synchronous Communication, servislerin HTTP/gRPC gibi anlık çağrılarla haberleşmesidir.
...
Bir servis kısa ve anlık doğrulama için başka bir servisi HTTP/gRPC ile çağırabilir.
...
Timeout, retry ve circuit breaker olmadan kullanılmamalıdır.
...
```text
API -> RequestService -> ApprovalService
```
