...
| Dosya Yolu | `docs/microservice/api-access/gateway-aggregation.md` |
...
Gateway Aggregation, birden fazla servisten veri toplayıp tek cevap döner.
...
Dashboard cevabı için birden fazla servisten veri alınıp tek response döndürülür.
...
Client tarafındaki chattiness azaltılır.
...
```text
Gateway -> RequestService + AuditService + ReportingService -> DashboardResponse
```
