...
| Dosya Yolu | `docs/microservice/api-access/backend-for-frontend.md` |
...
Backend for Frontend, her frontend tipi için özel backend katmanı oluşturur.
...
Web dashboard ile mobile uygulamanın veri ihtiyaçları farklıysa ayrı BFF katmanları hazırlanır.
...
Her frontend için optimize edilmiş API yüzeyi sağlar.
...
```text
Web UI -> Web BFF | Mobile UI -> Mobile BFF
```
