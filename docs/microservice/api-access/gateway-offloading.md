...
| Alt Kategori | API Access Patterns |
| Pattern | Gateway Offloading |
| Dosya Yolu | `docs/microservice/api-access/gateway-offloading.md` |
...
Gateway Offloading, sSL, auth, rate limit gibi ortak işleri gateway’e taşır.
...
SSL termination, authentication, rate limit ve request logging gibi işler servislerden gateway’e taşınır.
...
Servis kodunu sadeleştirir; gateway’i kritik bileşen haline getirir.
...
```text
Client -> Gateway Common Concerns -> Service
```
